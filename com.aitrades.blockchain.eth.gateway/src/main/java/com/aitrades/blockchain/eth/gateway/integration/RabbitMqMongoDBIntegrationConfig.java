...
@EnableIntegration
public class RabbitMqMongoDBIntegrationConfig {
	...
	@Bean
	@Autowired
	public IntegrationFlow mongoDbToRabbitQueue(MongoDatabaseFactory mongo) {
		return IntegrationFlows.from(mongoInboundSource(mongo), c -> c.poller(poller()))
	...
	@Bean
	@Autowired
	public MessageSource<Object> mongoInboundSource(MongoDatabaseFactory mongo) {// {'side' : 'buy'} // { qty: { $in: [ 5, 15 ] } } //  { $or: [ { 'status': 'A' } , { age: 50 } ] }
		MongoDbMessageSource messageSource = new MongoDbMessageSource(mongo, new LiteralExpression(" { 'orderCode': { $in: [ '83', '84' ] } }"));
